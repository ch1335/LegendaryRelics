...
public class CurioInventoryCapabilityMixin {
    ...
    private int ModifyFortuneLevel(int original) {
        ...
        Optional<SlotEffectHolder<OreCollectorEffect>> pairOptional = EquipmentEffectAPI.findBestEffect(this.livingEntity, LREquipmentEffectTypes.ORE_COLLECTOR_EFFECT.value());
        if (pairOptional.isPresent()) {
            SlotEffectHolder<OreCollectorEffect> pair = pairOptional.get();
            CalculatorArg args = CalculatorArg.simpleArg(livingEntity, pair.infoHolder().itemStack(), pair.infoHolder().effect());
            int addition = pair.infoHolder().itemStack().getOrDefault(LRDataComponentTypes.COLLECTED_MINERALS, CollectedMinerals.empty()).getCollectedOresAmount() >= OreCollectorEffect.COLLECTED_MINERALS_REQUIRE.getInt(args) ? 1 : 0;
