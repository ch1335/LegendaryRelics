...
public class SacredTalisman extends LRCuriosBase {
    ...
    public static void onDamage(CalculatorArg args, Player player, ItemStack itemStack) {
        if (!player.getCooldowns().isOnCooldown(LRItems.SACRED_TALISMAN.get())) {
            if (player.getHealth() <= player.getMaxHealth() * SacredTalisman.MAX_HEALTH_PERCENTAGE.getValue(args)) {
    ...
    public static void handleLivingIncomingDamageEvent(LivingIncomingDamageEvent event, CalculatorArg args, LivingEntity living, ItemStack itemStack) {
